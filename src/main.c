...
void earlyUpdate(l_engineData* pEngineData) {
	...
	inputData->moveInputDirection.x = 
		l_input_getKey(renderer, GLFW_KEY_A) - 
		l_input_getKey(renderer, GLFW_KEY_D);

	inputData->moveInputDirection.y = 
		l_input_getKey(renderer, GLFW_KEY_LEFT_SHIFT) - 
		l_input_getKey(renderer, GLFW_KEY_SPACE);

	inputData->moveInputDirection.z = 
		l_input_getKey(renderer, GLFW_KEY_S) - 
		l_input_getKey(renderer, GLFW_KEY_W);

	inputData->moveInputDirection = 
		blib_vec3f_normalize(inputData->moveInputDirection);
	// blib_vec3f_printf(moveInputDirection, "moveInputDirection");

	inputData->mouseDelta = blib_vec2f_subtract(
			inputData->lastMousePosition, 
			inputData->mousePosition);
	inputData->lastMousePosition = inputData->mousePosition;

	double xpos, ypos;
	glfwGetCursorPos(renderer->window, &xpos, &ypos);
	inputData->mousePosition.x = 
		(float) xpos - renderer->windowWidth * 0.5f;
	inputData->mousePosition.y = 
		(float) ypos - renderer->windowHeight * 0.5f;
...
void update(l_engineData* pEngineData) {
	...
	{ //light source cube update
		...
		l_renderer_gl_shader_setUniform3f(
				lightSourceCube->shader, 
				"u_lightColor",lightColor ,lightColor, lightColor 
				);

		glUniform1i(
				glGetUniformLocation(
					lightSourceCube->shader, "i_texCoord"
					), 0
				);
		...
		lightSourceCube->modelMatrix = l_renderer_gl_transform_GetMatrix(
				&lightSourceCube->transform
				);
	...
	{ //cube update
		...
		glUniform3f(
				glGetUniformLocation(
					pEngineData->cube.shader, "u_lightPosition"
					), 
		...
		glUniform1i(
				glGetUniformLocation(
					pEngineData->cube.shader, "i_texCoord"
					), 
				0
				);
		l_renderer_gl_shader_setMat4Uniform(
	...
	l_engineData engineData = (l_engineData) {
		.inputData = (l_inputData){0},
			...
			.cube = (l_cubeData) {
				...
				.transform = l_renderer_gl_transform_create(),
				.modelMatrix = l_renderer_gl_transform_GetMatrix(
						&engineData.cube.transform
						),
			}
	};


	l_renderer_gl_transform* lightTransform = 
		&engineData.lightsourcecube.transform;
