...
l_verlet_t l_verlet_alloc(lgl_batch_t batch) {
  ...
  verlet.position_old =
      calloc(sizeof(*verlet.position_old), batch.object.count);
...
void l_verlet_update(lgl_batch_t batch, l_verlet_t points) {
  ...
  for (unsigned int i = 0; i < points.count; i++) {
    ...
    vector3_t velocity = vector3_subtract(batch.object.transform.position[i],
    ...
    batch.object.transform.position[i] =
        vector3_add(batch.object.transform.position[i], velocity);
...
void l_verlet_constrain_distance(lgl_batch_t batch, l_verlet_t verlet,
  ...
  vector3_t diff = vector3_subtract(batch.object.transform.position[point_b],
  ...
  if (!verlet.is_pinned[point_b]) {
    batch.object.transform.position[point_b] =
        vector3_add(batch.object.transform.position[point_b], offset);
  }
}
