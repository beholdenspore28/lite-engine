...
void lite_engine_gl_start(void) {
  ...
  if (internal_gl_context->window_fullscreen) {
	...
	internal_gl_context->window_size_x, internal_gl_context->window_size_y,
	internal_gl_context->window_title, glfwGetPrimaryMonitor(), NULL);
  } else {
    internal_gl_context->window = glfwCreateWindow(
	internal_gl_context->window_size_x, internal_gl_context->window_size_y,
  ...
  { // projection
    {
      int window_size_x;
      int window_size_y;
      glfwGetWindowSize(internal_gl_context->window, &window_size_x,
			&window_size_y);
