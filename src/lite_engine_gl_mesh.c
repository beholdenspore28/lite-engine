...
void lgl_mesh_update(lgl_state_t state) {
	...
	for (ui64 e = 0; e < LITE_ENGINE_ENTITIES_MAX; e++) {
		...
		{ // draw
			...
			lgl_transform_calculate_matrix(
					&state.transforms[e]);

			lgl_shader_setUniformM4(
			...
			lgl_shader_setUniformM4(
					...
					&state
					.transforms[lgl_get_active_camera()]
					.matrix);

			// projection matrix uniform
			lgl_shader_setUniformM4(
					state.materials[e].shader, "u_projectionMatrix",
					&state.cameras[lgl_get_active_camera()]
					.projection);

			// camera position uniform
			lgl_shader_setUniformV3(
					state.materials[e].shader, "u_cameraPos",
					state.transforms[lgl_get_active_camera()]
					.position);
...
mesh_t lgl_mesh_lmod_alloc(const char *file_path) {
	...
	mesh_t mesh = lgl_mesh_alloc(vertices, indices);
	return mesh;
}
