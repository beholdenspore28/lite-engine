...
void lite_engine_gl_mesh_update(lite_engine_gl_state_t state) {
  ...
  for (ui64 e = 0; e < LITE_ENGINE_ENTITIES_MAX; e++) {
    ...
    { // draw
      ...
      lite_engine_gl_shader_setUniformM4(state.materials[e].shader,
					 "u_modelMatrix",
      ...
      const int light = 0;
      // light uniforms
      lite_engine_gl_shader_setUniformV3(state.materials[e].shader,
					 "u_light.position",
					 state.transforms[light].position);

      lite_engine_gl_shader_setUniformFloat(state.materials[e].shader,
					    "u_light.constant",
      ...
      lite_engine_gl_shader_setUniformInt(state.materials[e].shader,
					  "u_material.diffuse", 0);
      lite_engine_gl_shader_setUniformInt(state.materials[e].shader,
					  "u_material.specular", 1);
      lite_engine_gl_shader_setUniformFloat(state.materials[e].shader,
					    "u_material.shininess", 32.0f);
      lite_engine_gl_shader_setUniformV3(state.materials[e].shader,
					 "u_ambientLight", vector3_one(0.4));

      // draw
      glBindVertexArray(state.meshes[e].VAO);
      glDrawElements(GL_TRIANGLES, state.meshes[e].indices.length,
		     GL_UNSIGNED_INT, 0);
    }
  }
...
mesh_t lite_engine_gl_mesh_lmod_alloc(const char *file_path) {
  ...
  for (char *c = fb.text; c < fb.text + fb.length; c++) {
    ...
    if (strcmp(token, "vertex_texture_coordinates:") == 0 ||
	...
	c += sizeof("vertex_texture_coordinates:");
      ...
      if (num_tokens != 2) {
	debug_error("Failed to read vertex_texture_coordinates at '%s'",
		    file_path);
	assert(0);
      }
	...
	c += sizeof("vertex_positions:");
      ...
      int	num_tokens =
	  sscanf(c, "%f %f %f", &position.x, &position.y, &position.z);
      if (num_tokens != 3) {
